...
void NetworkHelper::post( const QString& url, const QString& content,const QString& SOAPAction, ReceiveFun receiver, void* data, int timeout /*= LimitTimeOut*/ )
{
	NetworkHelper* p = new NetworkHelper( url, content ,receiver, data, timeout );
    p->m_SOAPAction = SOAPAction;
	p->excutePost();
}
...
{
	QNetworkRequest network_request;
    network_request.setHeader(QNetworkRequest::ContentTypeHeader, "text/xml; charset=utf-8");//二进制流类型

    network_request.setRawHeader(QString("SOAPAction").toUtf8(),m_SOAPAction.toUtf8());
    network_request.setUrl( QUrl(m_url) );
